...
const HEADERS = [
  { label: 'Rule Name', icon: swapVert, flex: true },
  { label: 'Services', icon: filterAlt, flex: true },
  { label: 'Condition', icon: swapVert, flex: true },
  { label: 'Event Type', icon: filterAlt, flex: true },
  { label: 'Timestamp', icon: filterAlt, flex: true },
];
...
export default function AlertsTable({ rules, onRowClick, onEdit, onDelete }: Props) {
  return (
    <div className="flex w-full flex-col items-start overflow-clip rounded-2xl border-[0.5px] border-secondary-2 bg-white shadow-table">
      <div className="flex h-[54px] w-full items-start bg-secondary-6">
        {HEADERS.map((h) => (
          <div
            key={h.label}
            className="flex flex-1 items-center gap-1 bg-secondary-1 px-2 py-4 first:pl-6"
          >
            <p className="whitespace-nowrap text-sm font-semibold text-secondary-6">{h.label}</p>
            <img src={h.icon} alt="" className="size-4" />
          </div>
        ))}
        <div className="flex w-[88px] shrink-0 items-center justify-end gap-1 bg-secondary-1 px-2 pl-2 pr-6 py-4">
      ...
      {rules.map((rule) => (
        <div
          ...
          className="flex w-full cursor-pointer items-start border-b border-secondary-2 last:border-b-0 hover:bg-secondary-1/60"
        >
          ...
          <div className="flex h-[76px] w-[88px] shrink-0 items-center justify-end gap-1 overflow-clip py-3.5 pl-2 pr-6">
            <button
              ...
              className="rounded-full p-1 hover:bg-secondary-2"
            >
